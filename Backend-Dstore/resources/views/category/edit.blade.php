                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="status">
                                <option
                                    value="active" {{ old('status', $categoryById->status ?? '') == 'active' ? 'selected' : '' }}>
                                    Hoạt động
                                </option>
                                <option
                                    value="inactive" {{ old('status', $categoryById->status ?? '') == 'inactive' ? 'selected' : '' }}>
                                    Không hoạt động
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <select class="form-select" name="parent_id">
                            ...
                            @foreach($categories as $parent)
                                <option
                                    value="{{ $parent->id }}" {{ old('parent_id', $categoryById->parent_id ?? '') == $parent->id ? 'selected' : '' }}>
                                    {{ $parent->name }}
                                </option>
                                @if ($parent->children)
                                    @foreach ($parent->children as $child)
                                        <option
                                            value="{{ $child->id }}" {{ old('parent_id', $categoryById->parent_id ?? '') == $child->id ? 'selected' : '' }}>
                                            - {{ $child->name }}
                                        </option>
                                    @endforeach
                                @endif
